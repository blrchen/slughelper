...
{
    public class PinyinConveringRadionButton : SlugGeneratingRadioButton
    {
        protected override string GeneratingSlug(BlogPost post)
        {
            return GetPinyin(post.Title);
        }

        static private Hashtable _pinyinHash;

        #region static ctor.
        static PinyinConveringRadionButton()
        {
        ...
        /// <summary>
        /// 获得汉字的拼音，如果输入的是英文字符将原样输出，中文标点符号将被替换为-
        /// </summary>
        /// <param name="chineseChars">汉字字符串</param>
        /// <returns>拼音</returns>
        private static string GetPinyin(string chineseChars)
        {
        ...
        /// <summary>
        /// 获得汉字拼音的简写，即每一个汉字的拼音的首字母组成的串，如果输入的是英文字符将原样输出，中文标点符号被替换为-
        /// </summary>
        /// <param name="chineseChars">汉字字符串</param>
        /// <returns>拼音简写</returns>
        private static string GetShortPinyin(string chineseChars)
        {
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(chineseChars);
            int byteValue;
            StringBuilder sb = new StringBuilder(chineseChars.Length * 4);
